...
export function buildLlmsTxt({ compact = false } = {}) {
  ...
  lines.push('- Demo and contact workflows via POST /api/leads; read-only public content API documented at /developers')
  lines.push('')
  lines.push('## When agents should call the public API')
  lines.push('')
  lines.push('- Use GET /api/public/locale-content/erp (or /solutions, /faqs) to fetch published ERP programme copy programmatically')
  lines.push('- Use GET /api/public/blog/posts and /api/public/testimonials for published insights and social proof')
  lines.push('- Use GET /api/site-settings for company contact details (email, phone, address)')
  lines.push('- Use GET /api/public/seo-page?path=... to resolve canonical URLs, hreflang, and robots metadata')
  lines.push('- Use POST /api/leads only for human demo/contact submissions (rate limited; no API key today)')
  ...
  lines.push('- Demo/contact: POST /api/leads (public, rate limited, documented in /openapi.json)')
  lines.push('')
