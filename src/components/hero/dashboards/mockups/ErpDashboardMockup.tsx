...
export const ErpDashboardMockup = memo(function ErpDashboardMockup({ animate = false }: DashboardMockupProps) {
  ...
  const labels = data.erpRevenueTarget.labels.slice(0, 5)
  const actual = data.erpRevenueTarget.actual.slice(0, 5)
  ...
  return (
    <DashboardFrame moduleType="erp" title="ERP Overview" subtitle="Executive business dashboard">
      <DashboardBody>
        <KpiStrip
          items={[
            { label: 'Revenue', value: data.erpRevenueKpi, hint: '+8.2% vs last month', tone: 'up' },
            { label: 'Gross Profit', value: data.erpGrossKpi, hint: '+11.3% margin growth', tone: 'up' },
            { label: 'Receivables', value: data.erpReceivablesKpi, hint: 'Due within 30 days', tone: 'warn' },
          ]}
        />
        <div className="dm-hero__split dm-hero__split--2">
          ...
          <Panel title="Branch Performance">
            <BranchProgressList rows={data.erpBranchPerformance.slice(0, 4)} />
          </Panel>
        </div>
      </DashboardBody>
    </DashboardFrame>
