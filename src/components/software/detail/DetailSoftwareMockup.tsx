...
function buildMockups(
  m: ReturnType<typeof useDashboardRegionalData>,
): Record<DetailMockupVariant, MockupSpec> {
  const city = m.cities[0]
  const currency = m.currency
  const small = `${currency} 1,240`
  const med = m.erpReceivablesKpi
  const large = m.financeCashKpi
  const posToday = m.posTodayKpi
  const posBasket = m.posBasketKpi
  const stock = m.inventoryKpis[1]?.value ?? `${currency} 125K`
  const payroll = m.hrKpis[3]?.value ?? `${currency} 42K`
  const wip = m.erpGrossKpi
  const revenue = m.erpRevenueKpi
  ...
  return {
    petrol: {
      ...
      kpis: [
        ...
        { label: 'Nozzle sales', value: posToday, delta: 'Shift A' },
        { label: 'Variance', value: '0.08%', delta: 'Within limit' },
      ],
      rows: [
        { left: 'Tank T-01 · Diesel', right: '18,420 L', status: 'ok' },
        { left: 'Nozzle N-04 · Petrol', right: small, status: 'ok' },
    ...
    accounts: {
      ...
      kpis: [
        { label: 'Cash balance', value: large, delta: '+4.1%' },
        { label: 'Receivables', value: med, delta: '12 open' },
        { label: 'Bank unreconciled', value: small, delta: '3 items' },
      ],
      rows: [
        { left: 'Cash receipt · INV-1042', right: small, status: 'ok' },
        { left: 'Supplier payment · PO-88', right: med, status: 'ok' },
        { left: 'Bank reconciliation', right: 'Pending', status: 'pending' },
        { left: 'P&L preview · May', right: `${wip} net`, status: 'ok' },
      ],
      sidebarActive: 1,
    },
    inventory: {
      ...
      kpis: [
        ...
        { label: 'Stock value', value: stock, delta: '+2.3%' },
      ],
      rows: [
    ...
    pos: {
      ...
      kpis: [
        { label: 'Today sales', value: posToday, delta: '+8.4%' },
        { label: 'Transactions', value: '412', delta: 'Live' },
        { label: 'Avg ticket', value: posBasket, delta: '+2%' },
      ],
      rows: [
        { left: 'Top product · Engine oil 1L', right: posBasket, status: 'ok' },
    ...
    payroll: {
      ...
      kpis: [
        ...
        { label: 'Payroll run', value: payroll, delta: 'Draft' },
    ...
    textile: {
      ...
      kpis: [
        ...
        { label: 'WIP value', value: wip, delta: 'Live' },
      ],
      rows: [
    ...
    agriculture: {
      ...
      rows: [
        ...
        { left: 'Farm P&L preview', right: revenue, status: 'ok' },
      ],
      sidebarActive: 2,
    },
    poultry: {
      ...
      rows: [
        ...
        { left: 'Sales dispatch', right: small, status: 'ok' },
      ],
      sidebarActive: 2,
    },
    crm: {
      ...
      rows: [
        { left: `Lead · ${city} distributor`, right: 'Qualified', status: 'ok' },
    ...
    'generic-industry': {
      ...
      kpis: [
        ...
        { label: 'Today revenue', value: revenue, delta: '+3.8%' },
  ...
  return (
    <div className="accounts-proto-mockup" aria-hidden>
      <div className="accounts-proto-mockup__chrome">
        ...
        <span className="accounts-proto-mockup__url">{appUrl}</span>
      </div>
